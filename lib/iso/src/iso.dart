...
class Iso {
  ...
  Iso(this.runFunction, {this.onDataOut, this.onError})
    ...
    onError ??= (dynamic err) => throw IsolateRuntimeError("Error in isolate:\n $err");
  }
  ...
  final _isolateReadyToListenCompleter = Completer<void>();
  ...
  Future<void> get onCanReceive => _isolateReadyToListenCompleter.future;
  ...
  void send(dynamic data) {
    assert(_toIsolateSendPort != null, '_toIsolateSendPort != null');
  ...
  Future<void> run([List<dynamic> args = const <dynamic>[]]) async {
    ...
    // run
    await Isolate.spawn(runFunction, runner, onError: _fromIsolateErrorPort.sendPort).then((Isolate _is) {
      _isolate = _is;
