...
class GeoSerie {
  ...
  GeoSerie({required this.name, required this.type, this.id, this.surface, this.boundary, this.centroid, List<GeoPoint>? geoPoints}) : this.geoPoints = geoPoints ?? <GeoPoint>[];
  ...
  GeoSerie.fromNameAndType({required this.name, required String typeStr, this.id})
  ...
  Map<String, dynamic> toMap({bool withId = true}) {
    ...
    final json = <String, dynamic>{"name": name, "type": typeStr, "surface": surface};
    if (withId) {
      json["id"] = id;
    }
    return json;
  ...
  String toGeoJsonFeatureString(Map<String, dynamic>? properties) => _buildGeoJsonFeature(type, properties ?? <String, dynamic>{"name": name});

  String _buildGeoJsonFeature(GeoSerieType type, Map<String, dynamic> properties) {
    var extra1 = "";
    var extra2 = "";
