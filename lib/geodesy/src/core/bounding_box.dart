...
class BoundingBox {
  ...
  static List<LatLng> calculateBoundingBox(LatLng centerPoint, num distanceInKm) {
    // Earth's radius in kilometers
    const num radiusOfEarth = 6371e3 / 1000;
    ...
    final topLeft = LatLng(topLat.toDouble(), leftLng.toDouble());
    final bottomRight = LatLng(bottomLat.toDouble(), rightLng.toDouble());
    return [topLeft, bottomRight];
  }

  /// Check if a given geo point is in the bounding box
  static bool isGeoPointInBoundingBox(LatLng l, LatLng topLeft, LatLng bottomRight) {
    return (bottomRight.latitude <= l.latitude && l.latitude <= topLeft.latitude) && (topLeft.longitude <= l.longitude && l.longitude <= bottomRight.longitude);
  }
}
